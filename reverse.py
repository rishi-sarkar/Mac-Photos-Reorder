...
def reverse():
    directory = "./change"  # Folder containing renamed files
    try:
        mapping_file = os.path.join(directory, "file_mapping.txt")
        if not os.path.exists(mapping_file):
            print("Mapping file 'file_mapping.txt' not found in the 'change' folder.")
            return

        with open(mapping_file, "r") as f:
            for line in f:
                number, original_name = line.strip().split(",", 1)
                current_name = os.path.join(directory, f"{number}{os.path.splitext(original_name)[1]}")
